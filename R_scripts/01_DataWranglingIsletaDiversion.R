...
#Combine the three gages and get a total diversion
PERCN <- PERCN[-(3696),]
...
IsletaDiv <- cbind(PERCN1, CHICN1, BELCN1)
IsletaDiv <- IsletaDiv[-c(3,5)]

IsletaDiv <- IsletaDiv %>% 
  rename(MeanPERCN = Mean_daily_discharge_cfs, MeanCHICN=Mean_daily_discharge_cfs.1, 
         MeanBELCN=Mean_daily_discharge_cfs.2) %>% 
  mutate(Tot_diversion_cfs = MeanPERCN + MeanCHICN + MeanBELCN )
